...
const deleteParticularTask = async () => {
  ...
  if (data.toString() != "" && data != []) {
    ...
    for (let i = 0; i < data.length; i++) {
      if (data[i].Email == email) {
        todoCopy = [...data[i].Todos];
        data[i].Todos = data[i].Todos.filter((ob) => ob.id !== id);
        if (data[i].Todos.length !== todoCopy.length) {
          tf2 = true;
        }
        tf1 = true;
      }
...
module.exports = deleteParticularTask;
